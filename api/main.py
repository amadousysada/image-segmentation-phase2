...
#         cfg = dict(cfg)
#         cfg.pop('mlp')
#         # ne touche qu'au strict nécessaire
#         if "drop_prop" in cfg:
#             sig = inspect.signature(KHEnc.__init__)
#             params = sig.parameters
#             if "drop_rate" in params: cfg["drop_rate"] = cfg.pop("drop_prop")
#             elif "dropout" in params: cfg["dropout"] = cfg.pop("drop_prop")
#             elif "dropout_rate" in params: cfg["dropout_rate"] = cfg.pop("drop_prop")
#             else: cfg.pop("drop_prop", None)
#         return cls(**cfg)
...
async def load_model():
    ...
    try:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            #     }
            # )
            # Télécharge le dossier "model" (qui contient MLmodel + dossier "data/")
            #local_model_dir = mlflow.artifacts.download_artifacts(artifact_uri=f"runs:/{conf.RUN_ID}/model")
            #savedmodel_dir = os.path.join(local_model_dir, "data")  # <- le vrai SavedModel est ici
            # Deux choix:
            # 1) obtenir un tf.keras.Model:
